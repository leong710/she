    ...
    async function bat_storeDept(){
        ...
        const result = await load_fun('bat_storeDept', bat_storeDept_value, 'return');   // load_fun的變數傳遞要用字串
        // 250730 解決儲存後不返回首頁的問題(要保留在原頁面)
        const { action, fun, content } = result;
        if (action === 'success') {
            swal(fun, content, action, { buttons: false, timer: 1000 }); // 1秒自動關閉
        } else { // 處理 warning、info、error
            swal(fun, content, action); // 手動保留在本頁
        }
    }
    // 250226 單筆刪除Dept資料
