...
// 變更作業健檢
    ...
    function show_change($request){
        $pdo = pdo();
        extract($request);
        $sql = "SELECT _s.id, _c.emp_id, _s.cname, _c._deptLogs, _c._content FROM `_change` _c
                LEFT JOIN _staff _s ON _c.emp_id = _s.emp_id ";
        // tidy query condition：初始查詢陣列
            $conditions = [];
            $stmt_arr   = [];    
        // 中間-組合變數
            if(isset($emp_id) && $emp_id != "All"){                         // 處理過濾 emp_id != All  
                $conditions[] = "_c.emp_id = ?";
                $stmt_arr[]   = $emp_id;
            }
        // 最後-組合變數
            if (!empty($conditions)) {
        ...
        try {
            ...
            foreach($result as $index => $staff){
                $result[$index]['_deptLogs'] = json_decode($result[$index]['_deptLogs'], true);
                $result[$index]['_content']  = json_decode($result[$index]['_content'],  true);
            }
            return $result;
