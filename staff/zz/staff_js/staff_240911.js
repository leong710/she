    ...
    // [p-0]
        ...
        async function updateShCondition(ShCondition_value, select_empId) {
            return new Promise((resolve) => {
                ...
                document.getElementById(`shCondition,${select_empId},${currentYear}`).insertAdjacentHTML('beforeend', inner_Value);
                ...
                // step2.處理[特檢資格]shIdentity欄位
                        ...
                        // })
                // 球型標籤方式：
                document.getElementById(`shIdentity,${select_empId},${currentYear}`).innerHTML = '';
                Object.entries(ShCondition_value).forEach(([item, value]) => {
                    if(value){
                        const inner_item = `<span class="badge rounded-pill bg-success">${item}</span>`;
                        document.getElementById(`shIdentity,${select_empId},${currentYear}`).insertAdjacentHTML('beforeend', inner_item);
                    }
                })
                resolve();  // 當所有設置完成後，resolve Promise
            });
