...
// // 2. 數據操作函數 (Data Manipulation Functions)
    // [p-0]
            ...
            // 240826 進行emp_id重複值的比對並合併
                ...
                await combined.forEach(item => {
                    if (uniqueStaffMap.has(item.emp_id)) {
                        ...
                        // 確保 shCondition 被初始化
                        let currentYear = String(new Date().getFullYear()); // 获取当前年份
        ...
        async function updateDOM(sh_value, select_empId, sh_key_up) {
            return new Promise((resolve) => {
                ...
                if(hired){
                    empData['shCondition']['newOne'] = checkNewOne(hired);
                }
                resolve();  // 當所有設置完成後，resolve Promise
            });
        }
        ...
        async function post_hrdb(emp_arr){
            ...
            if(emp_arr.length === 0){
                ...
                await Object(emp_arr).forEach((emp_i)=>{        // 分解參數(陣列)，手工渲染，再掛載dataTable...
                    ...
                    tr += `<td id="MONIT_LOCAL,${emp_i.emp_id}"></td> <td id="WORK_DESC,${emp_i.emp_id}"></td>
                           <td id="HE_CATE,${emp_i.emp_id}" class="HE_CATE"></td>
                           <td id="AVG_VOL,${emp_i.emp_id}"></td>
                           <td id="AVG_8HR,${emp_i.emp_id}"></td>`;
                    tr += `<td><input type="number" id="eh_time,${emp_i.emp_id}" name="eh_time" class="form-control" onchange="change_eh_time(this.id, this.value)" ></td>
                           <td id="NC,${emp_i.emp_id}"></td>`;
    ...
    // [p-2]
        // modal：[load_excel] 以下為上傳後"iframe"的部分
            ...
            // eventListener() step-3-3. p-2 監控按下[載入]鍵後----呼叫Excel載入
